...
#github下载CloudflareSpeedTest使用ghproxy代理（可能不稳定，优先直接下载）
PROXY=""
...
if [[ -z ${zone_api_token} && -z ${api_token} ]]; then
  IP_TO_CF=0
else
  IP_TO_CF=1
fi
...
if [ ! -f ${CloudflareST} ]; then
	...
	if [[ $get_arch =~ "x86_64" ]];then
	    ...
	    echo "下载地址: ${PROXY}$URL"
	    if wget -P ./cf_ddns/tmp/ ${PROXY}$URL; then
	        echo "下载成功，开始解压..."
	        if tar -zxf ./cf_ddns/tmp/CloudflareST_linux_*.tar.gz -C ./cf_ddns/tmp/; then
	            echo "解压成功，移动文件..."
	            mv ./cf_ddns/tmp/CloudflareST ./cf_ddns/tmp/ip.txt ./cf_ddns/tmp/ipv6.txt ./cf_ddns/
	            rm -rf ./cf_ddns/tmp/
	            echo "CloudflareST 安装完成"
	        else
	            echo "解压失败"
	        fi
	    else
